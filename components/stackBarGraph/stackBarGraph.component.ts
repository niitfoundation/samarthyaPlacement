...
@Component({
    selector: 'stack-bar-graph',
    templateUrl: './stackBarGraph.component.html',
    styleUrls: ['./stackBarGraph.component.css'],
    providers: [GraphDataService]
})

export class StackBarGraph implements OnInit {
    ...
    ngOnInit() {
        this.initMargins();
        this.initSvg();
        this.GraphDataService.getGraphData().subscribe(res => this.drawChart(res));
    }
    ...
    private initSvg() {
        ...
        this.z = d3Scale.scaleOrdinal()
            .range(["#fdb103", "#db172a", "#8d8d8d", "#3c3cdf"]);
    }
    ...
    private drawChart(data: any[]) {
        ...
        this.x.domain(data.map((d: any) => d.Profession));
        ...
        this.g.append("g")
            ...
            .data(d => d)
            .enter().append("rect")
            .attr("x", d => this.x(d.data.Profession))
            .attr("y", d => this.y(d[1]))
